<?php                                                          
 ...
 $sql = "SELECT DATE_FORMAT(scans.sdate,'%e.%c.%Y %T') AS sdate, LEFT(products.type, 2) AS type, operators.name AS operator, defects.idnok FROM defects JOIN scans ON defects.idscan = scans.idscan JOIN products ON scans.product = products.idproducts JOIN operators ON scans.operator = operators.idoperator JOIN noklist ON defects.idnok = noklist.idnok WHERE (sdate BETWEEN '{$_POST['datefrom']}' AND '{$dateto}') AND noklist.priority = 1 ORDER BY scans.sdate ASC";
 $res = mysql_query($sql);
 $i = 2;
 ...
 $objPHPExcel->getActiveSheet()->setTitle('Priorita 1');
 ...
 $dateto = $_POST['dateto']." 23:59:59";
 $sql = "SELECT DATE_FORMAT(scans.sdate,'%e.%c.%Y %T') AS sdate, LEFT(products.type, 2) AS type, operators.name AS operator, defects.idnok FROM defects JOIN scans ON defects.idscan = scans.idscan JOIN products ON scans.product = products.idproducts JOIN operators ON scans.operator = operators.idoperator JOIN noklist ON defects.idnok = noklist.idnok WHERE (sdate BETWEEN '{$_POST['datefrom']}' AND '{$dateto}') AND noklist.priority = 2 ORDER BY scans.sdate ASC";
 $res = mysql_query($sql);
 $i = 2;
 while ($r = mysql_fetch_assoc($res)) {   
  $objPHPExcel->getActiveSheet()->SetCellValue('A'.$i, $r['sdate']);
 ...
 $objPHPExcel->getActiveSheet()->setTitle('Priorita 2');
